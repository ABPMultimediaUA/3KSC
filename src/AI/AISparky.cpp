...
void AISparky::update(){
    ...
    float t_SPARKY_special_up_wideness = 1.0;
    float t_SPARKY_special_up_min_height = 4.0;
    float t_SPARKY_special_side_wideness = 4.0;
    float t_SPARKY_special_side_height = 2.0;
    float t_SPARKY_special_down_wideness = 2.0;
    float t_SPARKY_special_down_height = 4.0;

    for (int i = 0; i < t_playerCount; i++){
        //Sparky shall not take himself into consideration
        if (i == m_index)
            continue;
        ...
        if(abs(target_x-self_x) < t_SPARKY_special_up_wideness){
            if(abs(target_y-self_y) < t_SPARKY_special_up_min_height){
        ...
        /*************************************************************/
        if(abs(target_x-self_x) < t_SPARKY_special_side_wideness){
            if(abs(target_y-self_y) < t_SPARKY_special_side_height){
                t_closestBodyFraction = m_physicsManager->RaycastBetween(t_p1, t_p2);
        ...
        /*************************************************************/
        /** Check if an enemy is in range for a special attack down **/
        /*************************************************************/
        if(abs(target_x-self_x) < t_SPARKY_special_down_wideness){
            if(abs(target_y-self_y) < t_SPARKY_special_down_height){
                t_closestBodyFraction = m_physicsManager->RaycastBetween(t_p1, t_p2);

                // Check wether there is an object between the 2 characters
                if(t_closestBodyFraction >= 0.83f){ //If there is not an intersection to the raycast
                    m_specialDownRange = 1.0;
                }
            }
        }
    }
...
// Builds the tree containing Sparky's AI. Links every node afterwards.
void AISparky::buildTree(){
    // Read JSON
    std::ifstream t_file("assets/ai/ai_tree_sparky.json");
    ...
    for(int i=0; nodes[i]!=nullptr; i++){
        ...
        if(t_node_type == 0){
            ...
            else if(nodes[i]["name"] == "special_down_range"){
                t_data = &m_specialDownRange;
            }
